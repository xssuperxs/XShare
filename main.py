...
def update_stock_cache(stock_market):
    ...
    if stock_market == 0:
        ...
        stocks_codes = stocks_df['code'].to_list()
        ...
        for code in stocks_codes:
            if code in cache_df["code"].values:
                ...
                # A 股重新命名列  统一成英文的列名
                df = df.rename(columns=column_mapping_hist)

                # 判断是否需要回测
                df_tail_150 = df.tail(100)

                df_tail_150.insert(0, 'code', code)

                pd.concat([cache_df, df_tail_150], axis=0)
                print(cache_df)
